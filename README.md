# 🌐 idev-lib

**idev-lib** là thư viện ý tưởng đồ án về **Mạng máy tính & Truyền thông dữ liệu** và các lĩnh vực kỹ thuật liên quan.

Các đề tài được phân loại theo nhóm chuyên môn, mức độ và hình thức triển khai để thuận tiện cho việc tham khảo, so sánh và lựa chọn hướng đồ án.

## 📖 Giới thiệu

Repository bao phủ từ networking, security, system administration đến IoT, cloud, AI/ML, automation và DevOps.

Repository được xây dựng nhằm:

- Tổng hợp các hướng đề tài liên quan đến Mạng máy tính & Truyền thông dữ liệu.
- Phân loại đề tài theo từng lĩnh vực kỹ thuật.
- Giúp tìm kiếm và so sánh các hướng đề tài thuận tiện hơn.
- Hỗ trợ lựa chọn đề tài theo độ khó, hình thức triển khai và mục tiêu học tập/nghiên cứu.

## 📚 Kho đề tài

| # | Lĩnh vực | Nội dung chính |
| ---: | --- | --- |
| 01 | 🌐 [Hạ tầng mạng](./01-ha-tang-mang/) | Routing, Switching, LAN/WAN, Wireless, VPN, QoS, IPv6 |
| 02 | 🛡️ [An ninh mạng](./02-an-ninh-mang/) | Defense, IDS/IPS, Honeypot, Attacks, Cryptography |
| 03 | 📊 [Giám sát & Quản trị](./03-giam-sat-quan-tri/) | Monitoring, Logging, SIEM, Traffic Analysis |
| 04 | 🤖 [AI & Machine Learning](./04-ai-ml/) | Anomaly Detection, Intrusion Detection, Security AI |
| 05 | 📡 [IoT & Wireless](./05-iot-wireless/) | IoT, Sensor Networks, MQTT, LoRaWAN, 5G/6G |
| 06 | ☁️ [Cloud & Virtualization](./06-cloud-virtualization/) | Cloud Platforms, Virtualization, Containers, Security |
| 07 | 🖥️ [System Administration](./07-system-admin/) | Linux, Windows, Network Services, Web/Mail, Storage |
| 08 | ⚙️ [Automation & DevOps](./08-automation-devops/) | Network Automation, Python, Ansible, IaC, CI/CD |
| 09 | 🏢 [Ứng dụng thực tế](./09-ung-dung-thuc-te/) | Enterprise, Healthcare, Education, Industry, Logistics |

## 🏷️ Thông tin đề tài

Mỗi file đề tài sử dụng bảng Markdown với các thông tin chính:

| Thuộc tính | Ý nghĩa |
| --- | --- |
| **STT** | Số thứ tự toàn cục, liên tục từ `1` đến `296`, dùng để định danh và sắp xếp đề tài trong kho |
| **Đề tài** | Tên hoặc hướng nghiên cứu chính của đề tài |
| **Mức độ** | Độ khó tương đối: `Cơ bản`, `Trung bình`, `Khó` |
| **Loại triển khai** | Hình thức hoặc môi trường triển khai phù hợp |

Các giá trị `Loại triển khai` dùng trong kho:

```text
Lab / Mô phỏng
Mạng thực nghiệm
Phần mềm / Server
Cloud / Virtualization
Phần cứng / IoT
AI / Dataset
Hybrid
```

> **Lưu ý:** Mức độ và loại triển khai mang tính định hướng; độ khó thực tế có thể thay đổi tùy theo phạm vi và mức độ mở rộng của từng đề tài.

## 📝 Ghi chú

Các đề tài trong repo là hướng tham khảo ban đầu. Khi chọn một đề tài cụ thể, nên tiếp tục xác định rõ phạm vi, yêu cầu kỹ thuật, môi trường triển khai và tiêu chí đánh giá trước khi thực hiện.
